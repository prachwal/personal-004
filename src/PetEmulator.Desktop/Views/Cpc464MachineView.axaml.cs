...
public partial class Cpc464MachineView : UserControl
{
    ...
    private void Rewire() { if (_wired is not null) _wired.FrameReady -= OnFrameReady; _wired = DataContext as Cpc464MachineViewModel; if (_wired is null) return; _wired.FrameReady += OnFrameReady; Screen.SetSize(_wired.PixelWidth, _wired.PixelHeight, 1, 1); Screen.UpdateFrame(_wired.FrameBuffer); Screen.Focus(); Log.LogInformation("Cpc464MachineView wired to {Title} ({Width}x{Height}).", _wired.WindowTitle, _wired.PixelWidth, _wired.PixelHeight); }
